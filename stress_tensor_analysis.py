...
with h5py.File(f'{input_dir}projViscosityField-{vel_file_no}.h5', 'r') as f:
    viscosity = f['data'][:]  
    
# accessing velocity data to mesh
mesh.point_data['velocity'] = velocity*3.17e-10/1000 # convert cm/yr to m/s

mesh.point_data['viscosity'] = viscosity

print(viscosity.min(), viscosity.max())

# # plot velocity magnitude
# mesh.plot(scalars='velocity', clim=[0, 5], cmap=plt.cm.viridis.resampled(12))
# mesh.plot(scalars='viscosity', clim=[1e18, 1e25], cmap=plt.cm.viridis.resampled(12),
#           log_scale=True)
# -
...
stress_tensor = velocity_gradient*viscosity # 1/yr to 1/s 
...
for i in range(N):
    ...
    if compute_eigen_3d:
        ...
        G = stress_tensor[i].reshape((3, 3))
    else:
        G = stress_tensor[i].reshape((3, 3))[:2, :2]
...
mesh["principal_stress_1"] = principal_stress[:, 0]
mesh["principal_stress_2"] = principal_stress[:, 1]
if compute_eigen_3d:
    mesh["principal_stress_3"] = principal_stress[:, 2]
...
mesh['stress_zz'] = stress_tensor[:,8]/1e6
...
# p = pv.Plotter()
# p.add_mesh(mesh, scalars="principal_stress_1", cmap=plt.cm.coolwarm.resampled(10), 
#            show_edges=False, clim=[-4e-2, 4e-2])
# p.show()
...
p.add_mesh(slice_mesh, scalars="stress_zz", cmap=plt.cm.coolwarm.resampled(20), 
           clim=[-50, 50],)# show_edges=True)

glyphs_max = slice_mesh.glyph(orient="SHmax", scale=True, factor=2e-1)
p.add_mesh(glyphs_max, color="black")

glyphs_min = slice_mesh.glyph(orient="SHmin", scale=True, factor=2e-1)
...
delta = 24.0 # Choose a boundary margin (delta)
...
p = pv.Plotter()
p.add_mesh(slice_mesh, scalars="stress_zz", opacity=1.0, show_scalar_bar=False,
           cmap=plt.cm.PRGn.resampled(10), clim=[-50, 50], show_edges=False,)

sbar = p.add_scalar_bar(title="Stress zz", vertical=True, title_font_size=20, 
                 label_font_size=20, 
