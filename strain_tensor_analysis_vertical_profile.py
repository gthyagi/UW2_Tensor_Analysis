...
# glyphs_max = slice_mesh.glyph(orient="SHmax", scale=scale, factor=factor )
# p.add_mesh(glyphs_max, color="black")

# glyphs_min = slice_mesh.glyph(orient="SHmin", scale=scale, factor=factor )
...
def plot_scalar_with_shmax_shmin(
    ...
    if arrow_type!='velocity':
        # overlay ONLY the zero contour
        ...
        ax.clabel(zero_contour, fmt='0', inline=True, fontsize=8)
    ...
    if arrow_type=='shmax':
        ...
        # Unpack SHmax and SHmin components
        u_max, v_max = SHmax_c[0], SHmax_c[1]
        u_min, v_min = SHmin_c[0], SHmin_c[1]
        
        # Compute lengths and avoid division by zero
        len_max = np.hypot(u_max, v_max)
        len_min = np.hypot(u_min, v_min)
        len_max[len_max == 0] = 1.0
        len_min[len_min == 0] = 1.0
        
        # Normalize to unit vectors
        u_max_unit = u_max / len_max
        v_max_unit = v_max / len_max
        u_min_unit = u_min / len_min
        v_min_unit = v_min / len_min
        
        # Plot quiver arrows for SHmax (blue) and SHmin (red), unit‐length lines
        for (u, v, col) in [
            (u_max_unit, v_max_unit, "red"),
            (-u_max_unit, -v_max_unit, "red"),
            (u_min_unit, v_min_unit, "blue"),
            (-u_min_unit, -v_min_unit, "blue"),
        ]:
            ax.quiver(
                X_c, Y_c,
        ...
        q = ax.quiver(
                        X_c, Y_c,
                        v_c[0], v_c[1],
                        color="k", scale=scale, width=0.006, 
                        headwidth=2.8, headlength=4.5, zorder=5,
                    )
