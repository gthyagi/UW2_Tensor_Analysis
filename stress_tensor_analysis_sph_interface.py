...
# Read the CSV file
data = pd.read_csv(output_dir+'inf_a25_swarm_stress_data.csv')
...
surface_mesh = point_cloud.delaunay_2d(alpha=0.005) # manual picking

# Plotting the result
plotter = pv.Plotter()
plotter.add_mesh(surface_mesh, color='lightblue', opacity=1., ) #show_edges=True)
# plotter.add_mesh(point_cloud, color='red', point_size=5)
plotter.show()

# # Visualize mesh with normals
# surface_normals = surface.compute_normals
# surface.plot_normals(mag=0.01, color='red')
# -
...
surface_stress_comps = data[['StressD_X', 'StressD_Y', 'StressD_Z', 'StressND_X', 'StressND_Y', 'StressND_Z']].values
...
# Plot
surface_mesh.plot(scalars="stress_inv", cmap=cm.hawaii_r.resampled(20), clim=[1e1, 4e4], cpos='xy')

# +
# # Assuming 'surface_mesh' has your vector data 'r_hat' already set:
# surface_mesh.set_active_vectors('r_hat')
# arrows = surface_mesh.glyph(orient='r_hat', scale=False, factor=0.01)

# plotter = pv.Plotter()
# plotter.add_mesh(arrows, color='r')
# plotter.add_mesh(surface_mesh, opacity=0.5, color='white')
# plotter.show()
# -
...
# Plot
surface_mesh.plot(scalars="principal_stress_1", cmap=plt.cm.coolwarm.resampled(10), clim=[-9e1, 9e1], cpos='xy')
...
# +
# plotting eigen vectors
glyph1 = surface_mesh.glyph(orient="SHmax", scale=False, factor=2e-3)
glyph2 = surface_mesh.glyph(orient="SHmin", scale=False, factor=2e-3)
# glyph3 = surface_mesh.glyph(orient="principal_dir_3", scale=True, factor=50)
...
# +
# Find indices where the green channel equals exactly 0.4
indices = np.where(np.around(surface_mesh["style_color"][:, 1], 6) == np.around(0.0, 6))[0]

# Extract subset mesh
subset_mesh = surface_mesh.extract_points(indices, adjacent_cells=False)

# Plotting
plotter = pv.Plotter()
plotter.add_mesh(subset_mesh, scalars="style_color", rgb=True, )
...
# surface_rot_glld = np.round(tc.uw_xyz2geo_lonlatr(surface_mesh.points), decimals=2)
# print(surface_rot_glld)
# surface_glld = rotate_arr(surface_rot_glld, rotate_angle, tc)
# print(surface_glld)

surface_rot_glld = np.round(tc.uw_xyz2geo_lonlatr(subset_mesh.points), decimals=2)
print(surface_rot_glld)
surface_glld = rotate_arr(surface_rot_glld, rotate_angle, tc)
print(surface_glld)
# -
...
# layer_coords_vel_list = [(surface_glld[:,0:2], surface_mesh['SHmax'][:,0:2])]
# color_list = [surface_mesh['style_color']]

layer_coords_vel_list = [(surface_glld[:,0:2], subset_mesh['SHmax'][:,0:2])]
color_list = [subset_mesh['style_color']]
# -
...
plot_field_data_bmrot(_rotated_crs=True, _p_lon=p_lon, _p_lat=p_lat, _ax_set_extent=ax_extent_rot, _left_labels=left_labels, _bottom_labels=bottom_labels, 
                      ...
                      _him_trench_coords=him_trench_coords, _layer_coords_vel_list=layer_coords_vel_list, _rotate_angle_list=[rotate_angle], _lvec_freq=30, 
...
plot_field_data(_p_lat=p_lat, _p_lon=p_lon, _ax_set_extent=ax_extent_rot, _rotated_crs=True,
                ...
                _layer_coords=surface_glld[:,0:2], _layer_vel=rotate_vec_arr(subset_mesh['SHmax'][:,0:2], rotate_angle), 
                _lvec_color=subset_mesh['style_color'], _lvec_scale=75, _lvec_freq=30)
